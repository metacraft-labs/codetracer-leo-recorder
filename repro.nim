## Reprobuild dev env for codetracer-leo-recorder.
##
...
##
## Leo-specific note: the test corpus consists of pre-compiled
## Aleo program artifacts, so the ``leo`` compiler itself is not in
## ``uses:``. If a future test grows a live leo-compile step,
## declare leo here (and add it to the reprobuild standard catalog
## first).
